...
class RecetteViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func enregistrerRecette(_ sender: Any) {
        ...
        // Pour chaque élément dans listeIngredient, créer une nouvelle entité de type Ingrédient
            for (nom, quantite) in listeIngredient {
                let ingr = Ingredient(entity: NSEntityDescription.entity(
                    forEntityName: "Ingredient", in:managedContext)!,
                    insertInto: managedContext)
                
                // Affecter les valeurs de l'ingrédient
                ingr.nom = nom
                ingr.quantite = Float(quantite) ?? 0.0
                
                // Ajouter l'ingrédient à la recette
                recette.addToEstComposee(ingr)
            }
            
            // Enregistrer la recette grâce aux méthodes du framework CoreData
            do {
                try managedContext.save()
                print("Recette enregistrée avec succès.")
            } catch let error as NSError {
                print("Impossible d'enregistrer la recette. Erreur : \(error), \(error.userInfo)")
            }
